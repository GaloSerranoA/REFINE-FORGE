...
- Added opt-in `--ranking best-first` to `refine inmortal-proof search`, which reorders the candidate queue by a deterministic hand-tuned prior heuristic over Lean tactic and value generators (block: `builtin_preserve` > `rfl` > `exact True.intro` > `trivial` > `simp` > `decide` > `simp_all`; value: `builtin_preserve` > `()` > `true` > `false` > `0` > `1`). Default remains `--ranking insertion-order`. Each candidate now carries a `heuristic_score` field in `candidates.jsonl`, and the report includes a `ranking_mode` field. This is a hand-tuned tactic prior, NOT a learned value function or policy network.
