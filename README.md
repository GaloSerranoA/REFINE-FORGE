...
| `refine approval draft --review-request <file>` | Role-agnostic approval draft flow for `training`, `kernel`, and `lean`; verifies role evidence and writes draft-only `approvals/<role>.draft.json` without creating final approval |
| `refine approval approve --i-reviewed-this-evidence` | Rerun role evidence validation and write `approvals/<role>.json` only for a named policy-allowed human operator |
