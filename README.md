...
| [docs/inmortal-proof.md](docs/inmortal-proof.md) | InmortalProof proof-search substrate: protected Lean evolution regions, receipts, population seeds, goal cache, and AlphaProof boundary |
...
| InmortalProof proof-search substrate | ✅ `refine inmortal-proof run <id>` writes deterministic proof-search receipts with EVOLVE region parsing, full/protected source hashes, population seed, P-UCB selection receipt, episode ledger, goal cache, no-sorry policy status, and an explicit boundary that this is Refine-Forge's own substrate, not an AlphaProof/Gemini claim |
