...
| `refine approval draft --review-request <file>` | Role-agnostic approval draft flow for `training`, `kernel`, and `lean`; verifies role evidence, writes draft-only `approvals/<role>.draft.json`, and keeps the review request at `decision: pending` without creating final approval |
