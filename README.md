...
| `refine inmortal-proof search <id>`    | Execute bounded active search over EVOLVE regions. Default `--validator lean` runs the normal Lean gate and exports a bundle only after verification; `--validator receipt-only` is deterministic development mode and not a proof claim. |
