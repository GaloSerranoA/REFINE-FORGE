...
| `refine autonomous <id> [--strategy mock\|anthropic-mock\|anthropic] [--max-cost-usd 10] [--operator EMAIL] [--dry-run] [--auto-repair]` | Phase 3.6 of the [autonomous-driver-plan](docs/autonomous-driver-plan.md). Plans + executes the baseline workflow against a real claim YAML; system steps shell to `runner::run` / `scan::scan_claim` / `bundle::export`. ProjectContext is loaded from `claims/` + `lean/lake-manifest.json` + `Cargo.lock`. **`--auto-repair` + `--strategy anthropic` calls the live LLM** (needs `ANTHROPIC_API_KEY` in env); cost-gate fails closed. Escalates per criteria v0.3 (no auto-expiry). Writes `autonomous/runs/<CLAIM>-<timestamp>.json`. Still pending: `await_decision` resumption, `refine-train`/`refine-bitexact` integration, EXAMPLE-002 forced-Counter dogfood. |
