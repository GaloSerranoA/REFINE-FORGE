...
| `refine autonomous <id> [--strategy mock] [--max-cost-usd 10] [--operator EMAIL] [--dry-run]` | Phase 3.5 of the [autonomous-driver-plan](docs/autonomous-driver-plan.md). Plans + executes the baseline workflow against a real claim YAML; system steps shell to `runner::run` / `scan::scan_claim` / `bundle::export`. ProjectContext is loaded from `claims/` + `lean/lake-manifest.json` + `Cargo.lock`. Escalates per criteria v0.3 (no auto-expiry). Writes `autonomous/runs/<CLAIM>-<timestamp>.json`. Still pending: live Anthropic strategy, `await_decision` resumption, `refine-train`/`refine-bitexact` integration. |
