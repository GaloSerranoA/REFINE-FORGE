...
| [docs/inmortal-proof.md](docs/inmortal-proof.md) | InmortalProof proof-search: protected Lean evolution regions, active candidate search, receipts, rollback, bundle export on Lean verification, and AlphaProof boundary |
...
| 3 | ML / training engineer | `refine-train` dataset audit, SFT packing, causal-LM preprocessing, built-in `refineforge_native` and `refineforge_native_causal_lm` smoke training, HELYX/HRM/PyTorch orchestration, HRM-Text runtime probes/manifests, checkpoints, reports, evidence generation, local-finetune promotion, and Training Agent evidence validation | Refine-Forge can run local native smoke training and produce evidence packs; production model trust still requires real held-out eval, regression, compute ledger, conversion, promotion lineage, hash-verified runtime handoff, and human approval evidence |
...
└── docs/
    ...
    ├── inmortal-proof.md       # InmortalProof proof-search receipt substrate
...
| `refine inmortal-proof run <id>`       | Prepare an InmortalProof proof-search receipt pack: protected EVOLVE regions, source/protected hashes, population seed, P-UCB selection receipt, episodes ledger, goal cache, and explicit no-AlphaProof-overclaim boundary. See [`docs/inmortal-proof.md`](docs/inmortal-proof.md) |
| `refine inmortal-proof search <id>`    | Execute bounded active search over EVOLVE regions. Default `--validator lean` runs the normal Lean gate and exports a bundle only after verification; `--validator receipt-only` is deterministic development mode and not a proof claim. Opt-in `--ranking best-first` reorders candidates by a deterministic hand-tuned prior heuristic (not a learned value function). |
...
| `refine approval draft --review-request <file>` | Role-agnostic approval draft flow for `release`, `training`, `kernel`, and `lean`; verifies role evidence, writes draft-only `approvals/<role>.draft.json`, and keeps the review request at `decision: pending` without creating final approval |
| `refine approval approve --i-reviewed-this-evidence` | Rerun role evidence validation and write `approvals/<role>.json` only for a named policy-allowed human operator; resolved review requests record both `status: approved` and `decision: approved` |
...
| InmortalProof proof-search substrate | ✅ `refine inmortal-proof run <id>` writes deterministic proof-search receipts; `refine inmortal-proof search <id>` performs bounded built-in candidate generation, EVOLVE-only source edits, Lean/receipt validation, failed-candidate rollback, accepted-candidate promotion, JSONL ledgers, and bundle export only after Lean verification. Explicit boundary: this is Refine-Forge's own substrate, not an AlphaProof/Gemini claim |
