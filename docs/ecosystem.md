...
| **Data pipeline** | Knowledge-Foundry (KF) | `D:\AI-PROJECTS-GALO\PROJECTS\Knowledge-Foundry` | Generates training corpora via LLM teachers (Anthropic, etc.). 5 modes — `sft_pair`, `dpo_preference`, `cot_trace`, `embedding_triplet`, `tool_call`. Multi-teacher. HuggingFace publishing. **Produces the corpora HELYX's `helyx-train` consumes.** 330 source files; 1115 tests passing. **Python.** |
