...
run reports and acceptance evidence, not just a successful command. Execute
mode runs dataset audit plus `refine-train run --dry-run` by default; live
backend execution requires `--allow-expensive`.
