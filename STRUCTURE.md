...
├── inmortal-proof-runs/        # `refine inmortal-proof run/search` receipt packs (gitignored)
...
└── src/
    ...
    ├── nexus.rs            # `refine inmortal-proof run/search` — protected EVOLVE regions, active search, rollback, receipts
...
| `inmortal-proof.md` | Lean/ML maintainers | InmortalProof proof-search substrate, EVOLVE region contract, active candidate search, rollback, bundle export on Lean verification, deterministic receipts, goal cache, and AlphaProof boundary |
...
| `ecosystem.md` | operator, reviewers | **v0.2 (HELYX-as-LLM reframe).** HELYX is the LLM the operator is building; Cogn8ty + KF + refineforge are HELYX's infrastructure. refineforge exists because the alternative was hiring 4 engineers to verify HELYX's load-bearing claims manually. v0.1 had flat-4-peers framing; superseded |
