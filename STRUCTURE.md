...
| `ecosystem.md` | operator, reviewers | The 4-project portfolio map (Cogn8ty = symbolic NARS; HELYX+NAL = neural NARS + verified core; KF = data pipeline; refineforge = trust framework). Records the structural observation that Cogn8ty + HELYX-NAL are the two halves of NARS in the strict sense, both in pure Rust. Pair-by-pair complementarity matrix. Honest "complementary ≠ integrated" carve-out + 3 sensible next-step deliverables |
